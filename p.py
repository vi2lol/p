...
class C2Server:
    ...
    def _decrypt(self, data: bytes, shared_key: bytes, nonce: int) -> bytes:
        try:
            ...
            cipher = Cipher(algorithms.AES(shared_key), modes.GCM(iv, tag), default_backend())
            ...
            async for message in websocket:
                ...
                try:
                    ...
                    if self._verify_command(data, signature, client["hmac_key"]):
                        ...
                        log_buffer.append({"ts": time.time(), "level": "WARNING", "msg": f"Invalid signature from {client_id}"})
